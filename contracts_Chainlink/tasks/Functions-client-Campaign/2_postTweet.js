...
task("functions-postTweet", "Initiates a request from a Functions client contract")
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    const transactionEstimateGas = await clientContract.estimateGas.postTweet(
      requestConfig.secrets && Object.keys(requestConfig.secrets).length > 0 ? simulatedSecretsURLBytes : [],
      requestConfig.args ?? [],
      subscriptionId,
      gasLimit,
    ...
    const spinner = utils.spin({
     ...
     // Attach to the required contracts
    let forecastedCampaignBalance = await clientContract.forecastedCampaignBalance()
    console.log(`Before posting tweet forecastedBalance : ${forecastedCampaignBalance.div(BigNumber.from(10).pow(stcDecimals))} $STC \n`)
    ...
    await new Promise(async (resolve, reject) => {
      ...
      console.log(`\nRequesting new data for Campaign contract ${contractAddr} on network ${network.name}`)
      const requestTx = await clientContract.postTweet(
        request.secrets ?? [],
        request.args ?? [],
        subscriptionId,
        gasLimit,
      ...
      let forecastedCampaignBalance = await clientContract.forecastedCampaignBalance()
      console.log(`After posting tweet forecastedBalace : ${forecastedCampaignBalance.div(BigNumber.from(10).pow(stcDecimals))} $STC\n`)
    })
  })
